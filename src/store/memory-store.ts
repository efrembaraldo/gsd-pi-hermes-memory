...
export class MemoryStore {
	...
	private get memoryDir(): string {
		return (
			this.config.memoryDir ?? path.join(AGENT_ROOT, "gsd-pi-hermes-memory")
		);
	}
	...
	async remove(
		...
		return this.runTargetMutation(target, () =>
			this.removeUnlocked(target, oldText),
		);
	}
	...
	private async readFileState(
		filePath: string,
	): Promise<{ entries: string[]; fingerprint: string }> {
		try {
