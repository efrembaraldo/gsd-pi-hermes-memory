 ...
 *
 * Pi installs extension deps under ~/.gsd/agent/npm. When the host Node that
...
export class BetterSqlite3LoadError extends Error {
	...
	constructor(
		message: string,
		options: { packageRoot?: string | null; cause?: unknown } = {},
	) {
		super(message);
...
export function resolveBetterSqlite3PackageRoot(
	requireImpl: NodeRequire,
): string | null {
	try {
		...
		while (true) {
			...
			if (fs.existsSync(pkgPath)) {
				try {
					...
					if (pkg && typeof pkg === "object" && pkg.name === "better-sqlite3")
						return dir;
				} catch {
					// keep walking
				}
...
function defaultRebuild(packageRoot: string): { ok: boolean; detail: string } {
	...
	for (const attempt of attempts) {
		...
		if (result.status === 0) {
			return {
				ok: true,
				detail: (result.stdout || result.stderr || "rebuild ok").trim(),
			};
		}
		const output = [result.stdout, result.stderr]
			...
			.trim();
		details.push(
			`${attempt.command} exited ${result.status}${output ? `: ${output}` : ""}`,
		);
	}
...
function unwrapModule(
	mod: { default?: BetterSqlite3DatabaseCtor } | BetterSqlite3DatabaseCtor,
): BetterSqlite3DatabaseCtor {
	return (
		(mod as { default?: BetterSqlite3DatabaseCtor }).default ??
		(mod as BetterSqlite3DatabaseCtor)
	);
}

function clearBetterSqlite3RequireCache(
	requireImpl: NodeRequire,
	packageRoot: string | null,
): void {
	for (const key of Object.keys(requireImpl.cache)) {
		if (
			!key.includes(`${path.sep}better-sqlite3${path.sep}`) &&
			!key.endsWith(`${path.sep}better-sqlite3`)
		) {
			continue;
		}
...
export function formatBetterSqlite3AbiError(options: {
	...
	const location =
		options.packageRoot ?? "(better-sqlite3 package root unknown)";
	...
	return [
		"gsd-pi-hermes-memory could not load the native better-sqlite3 module for this Node runtime.",
		`Runtime: ${runtime}`,
		`Module: ${location}`,
		...
		options.packageRoot
			...
			: "  cd ~/.gsd/agent/npm && npm rebuild better-sqlite3",
...
export function loadBetterSqlite3(
	options: SqliteNativeLoadOptions = {},
): BetterSqlite3DatabaseCtor {
	const requireImpl =
		options.requireImpl ??
	...
	const loadOnce = (): BetterSqlite3DatabaseCtor => {
		const mod = requireImpl("better-sqlite3") as
			| { default?: BetterSqlite3DatabaseCtor }
			| BetterSqlite3DatabaseCtor;
	...
	try {
		...
		const canRebuild =
			options.allowRebuild ?? isNativeModuleAbiMismatch(firstError);
