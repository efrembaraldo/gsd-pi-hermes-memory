...
// Resolve the path to gsd-pi-hermes-memory's own extension entry point.
...
function effectiveThinkingOverride(
	...
	return (
		config.llmThinkingOverride ??
		(normalizedModelOverride(config) ? "off" : undefined)
	);
}
...
export function inheritedExtensionArgs(
...
// gsd-pi-hermes-memory's own package.json declares) and match sibling package
...
// default: packages installed alongside gsd-pi-hermes-memory's own package
// (the common npm-managed-extensions layout), and AGENT_ROOT's npm
// directory (covers gsd-pi-hermes-memory being loaded from elsewhere, e.g. a
		...
		try {
			const invocation = resolveWatchedChildPiInvocation(
				resolveChildPiInvocation(
					buildChildPiPromptArgs(promptReference, config),
				),
				options.timeoutMs,
				cancellationPath,
			);
