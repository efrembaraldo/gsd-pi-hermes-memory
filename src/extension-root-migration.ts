...
import {
	...
	type AtomicLockLease,
} from "./store/atomic-lock-coordinator.js";
	...
	return class BunMigrationDatabase implements MigrationDatabase {
		...
		constructor(
			...
			if (typeof options.timeout === "number") {
				this.db.exec(
					`PRAGMA busy_timeout = ${Math.max(0, Math.trunc(options.timeout))}`,
				);
			}
		}
...
export function isDatabaseMigrationPending(
	legacyRoot: string,
	targetRoot: string,
): boolean {
	return (
		existsSync(path.join(targetRoot, DATABASE_MIGRATION_PENDING_FILE)) ||
		(existsSync(path.join(legacyRoot, "sessions.db")) &&
			!existsSync(path.join(targetRoot, "sessions.db")))
	);
}
...
async function stageDatabaseSnapshot(
	...
	try {
		...
		if (check !== "ok")
			throw new Error(
				`staged SQLite snapshot failed integrity_check: ${String(check)}`,
			);
	} finally {
		stagedDb.close();
	}
...
async function acquireMigrationLease(
	...
	const targetIdentity = canonicalStoragePathSync(
		path.join(targetRoot, "sessions.db"),
	);
	...
	while (true) {
		...
		if (Date.now() >= deadline) {
			throw new Error(
				`SQLite extension-root migration already in progress for ${targetIdentity}`,
			);
		}
		await new Promise<void>((resolve) =>
			setTimeout(resolve, MIGRATION_LOCK_POLL_MS),
		);
	}
}
...
async function moveDatabaseGeneration(
	...
	const orderedNames = [...names].sort(
		(left, right) =>
			Number(left === "sessions.db") - Number(right === "sessions.db"),
	);
	try {
...
async function stageDatabaseSymlink(
	source: string,
	staged: string,
): Promise<void> {
	...
	if (before !== after)
		throw new Error("sessions.db symlink changed while staging");
}
...
async function moveDirContents(
	...
	for (const entry of entries) {
		if (
			!relativeDir &&
			DATABASE_FILES.includes(entry.name as (typeof DATABASE_FILES)[number])
		) {
			continue;
		}
...
async function migrateDatabaseGeneration(
	...
	try {
		const pendingMarker = path.join(
			targetRoot,
			DATABASE_MIGRATION_PENDING_FILE,
		);
		...
		if (sourceNames.length === 0) {
			...
			result.warnings.push(
				`${path.join(legacyRoot, "sessions.db")}: ${message}`,
			);
			result.criticalFailures.push({
				name: "sessions.db",
				source: path.join(legacyRoot, "sessions.db"),
		...
		const stagingDir = path.join(
			targetRoot,
			`.sessions-db-migration-${randomUUID()}`,
		);
		const retirementDir = path.join(
			legacyRoot,
			`.sessions-db-retirement-${randomUUID()}`,
		);
		...
		try {
			...
			try {
				writeLock = new (getDatabaseCtor())(source, {
					fileMustExist: true,
					timeout: 0,
				});
			...
			if (sourceState.isSymbolicLink()) {
				if (sourceNames.length !== 1) {
					throw new Error(
						"symlinked sessions.db cannot be combined with legacy SQLite sidecars",
					);
				}
				...
				if (corruptGeneration) {
					try {
						...
						if (error instanceof DatabaseGenerationMoveError)
							retired = error.moved;
						throw error;
					}
			...
			if (retired.length > 0) {
				restoreFailures = await restoreDatabaseGeneration(
					retired,
					retirementDir,
					legacyRoot,
				);
			...
			const destinationPreserved = await pathEntryExists(
				path.join(targetRoot, "sessions.db"),
			);
...
export async function migrateExtensionRoot(
	...
	if (result.criticalFailures.some((failure) => failure.name === "sessions.db"))
		return result;
	await moveDirContents(
		legacyRoot,
		targetRoot,
		result,
		options.moveFile ?? moveFileSafe,
	);
