...
function isLegacyProjectDir(
	...
	// Both the current extension root (gsd-pi-hermes-memory) and the previous
	// one (pi-hermes-memory, before the v0.9.4 rename) are non-project dirs and
	// must not be scanned as legacy project memory sources.
	if (
		name === "memory" ||
