...
describe("registerMemoryTool", () => {
	...
	it("registers tool with name 'memory' and correct parameters", () => {
		...
		assert.strictEqual(
			...
			1,
			"should register exactly one tool",
		);
		...
		assert.ok(
			...
			"promptSnippet should not be empty",
		);
		assert.ok(
			Array.isArray(tool.promptGuidelines),
			"promptGuidelines should be an array",
		);
	...
	it("execute add with FIFO evictions returns normal text with full rotated entries", async () => {
		...
		const mockStore = {
			add: () => ({
				...
				entry_count: 1,
				message:
					"Memory updated. Rotated 2 older entries to stay within the limit.",
		...
		assert.match(
			text,
			/If one of these entries should stay active, add it again\./,
		);
		assert.match(text, /Usage: 90%/);
		assert.deepStrictEqual(result.details.evicted_entries, [
			evictedOne,
			evictedTwo,
		]);
	});
	...
	it("prunes same-scope SQLite orphans after a Markdown mutation", async () => {
		...
		await capturedResult.execute(
			"tc-1",
			{
				action: "add",
				target: "memory",
				content: "authoritative Markdown row",
			},
			undefined,
			undefined,
			undefined,
		...
		assert.deepStrictEqual(
			getMemories(dbManager, { target: "memory", project: null }).map(
				(entry) => entry.content,
			),
	...
	it("reconciles SQLite from fresh authoritative Markdown state", async () => {
		...
		(store as any).saveToDisk = async (target: "memory") => {
			...
			fs.writeFileSync(
				markdownPath,
				`${existing}${ENTRY_DELIMITER}newer writer <!-- created=${date}, last=${date} -->`,
			);
			syncMemoryEntry(dbManager, {
				content: "newer writer",
				target: "memory",
				project: null,
			});
		};
	...
	it("removes FIFO-evicted entries from the SQLite mirror", async () => {
		...
		const mockStore = {
			add: () => ({
				...
				entry_count: 1,
				message:
					"Memory updated. Rotated 1 older entry to stay within the limit.",
	...
	it("uses project scope when removing FIFO-evicted SQLite entries", async () => {
		...
		const globalRows = getMemories(dbManager, {
			target: "memory",
			project: null,
		});
	...
	it("does not sync to SQLite when core Markdown add fails", async () => {
		...
		const mockStore = {
			add: () => ({
				success: false,
				error:
					"Memory at 5000/5000 chars. Adding this entry would exceed the limit.",
			}),
		} as unknown as MemoryStore;
		...
		assert.strictEqual(
			rows.length,
			0,
			"SQLite should stay unchanged when core add fails",
		);
	});
	...
	it("execute add without content returns error", async () => {
		...
		assert.ok(
			parsed.error.includes("required"),
			"error should mention required content",
		);
	});
	...
	it("execute replace without old_text returns error", async () => {
		...
		assert.ok(
			parsed.error.includes("old_text"),
	...
	it("execute delegates remove to store.remove", async () => {
		...
		assert.deepStrictEqual(
			removeArgs,
			["memory", "old entry"],
			"should pass target, old_text to store.remove",
		);
	});
});
