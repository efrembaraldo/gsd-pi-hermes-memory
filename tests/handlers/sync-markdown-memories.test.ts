...
describe("memory sqlite sync + markdown backfill", () => {
	...
	beforeEach(() => {
		tmpDir = fs.mkdtempSync(
			path.join(os.tmpdir(), "memory-sync-command-test-"),
		);
	...
	it("memory tool writes are immediately searchable in SQLite", async () => {
		...
		const results = searchMemories(dbManager, "sync token 2026-05-09", {
			target: "memory",
		});
	...
	it("backfill command is idempotent across repeated runs", async () => {
		...
		fs.writeFileSync(
			...
			memoryEntries.join(ENTRY_DELIMITER),
			"utf-8",
		);
		fs.writeFileSync(
			path.join(globalDir, "USER.md"),
			userEntries.join(ENTRY_DELIMITER),
			"utf-8",
		);
		fs.writeFileSync(
			path.join(globalDir, "failures.md"),
			failureEntries.join(ENTRY_DELIMITER),
			"utf-8",
		);
		...
		assert.strictEqual(
			afterFirst.length,
			5,
			"first run should import all unique entries",
		);
		assert.strictEqual(
			afterSecond.length,
			5,
		...
		const projectRows = getMemories(dbManager, {
			project: "project-a",
			target: "memory",
		});
		assert.strictEqual(projectRows.length, 1);

		const failureRows = getMemories(dbManager, {
			target: "failure",
			category: "tool-quirk",
		});
	...
	it("makes new-layout project markdown searchable when startup sync runs", async () => {
		...
		const counters = await syncMarkdownMemoriesToSqlite(
			dbManager,
			globalDir,
			undefined,
			agentRoot,
    ...
    `)
		...
		assert.deepStrictEqual(
			getMemories(dbManager)
				.map(
					(entry) =>
						`${entry.project ?? "global"}:${entry.target}:${entry.content}`,
				)
				.sort(),
			[
	...
	it("waits for the canonical Markdown mutation before reading and reconciling", async () => {
		...
		fs.writeFileSync(
			memoryFile,
			["stale memory", "newer writer memory"].join(ENTRY_DELIMITER),
			"utf-8",
		);
		...
		assert.ok(
			waitedForMutation,
			"reconciliation must wait for the active Markdown mutation",
		);
		assert.deepStrictEqual(
			getMemories(dbManager, { target: "memory", project: null })
	...
	it("prunes a deleted project Markdown scope without touching unrelated rows", async () => {
		const deletedProjectDir = path.join(
			agentRoot,
			"projects-memory",
			"deleted-project",
		);
		const keptProjectDir = path.join(
			agentRoot,
			"projects-memory",
			"kept-project",
		);
		...
		fs.writeFileSync(
			path.join(deletedProjectDir, "MEMORY.md"),
			"deleted project memory",
			"utf-8",
		);
		fs.writeFileSync(
			path.join(keptProjectDir, "MEMORY.md"),
			"kept project memory",
			"utf-8",
		);
		fs.writeFileSync(
	...
	it("treats a missing canonical project file as empty despite a retained legacy backup", async () => {
		...
		assert.strictEqual(counters.removed, 1);
		assert.deepStrictEqual(
			getMemories(dbManager, { project: projectName, target: "memory" }),
			[],
		);
		assert.strictEqual(
	...
	it("reconciles unsafe SQLite project scopes empty without reading outside projects-memory", async () => {
		...
		fs.writeFileSync(
			path.join(outsideDir, "MEMORY.md"),
		...
		assert.strictEqual(counters.removed, 1);
		assert.deepStrictEqual(
			getMemories(dbManager, { project: "../outside", target: "memory" }),
			[],
		);
		assert.strictEqual(
			fs.readFileSync(path.join(outsideDir, "MEMORY.md"), "utf-8"),
			"outside traversal bait",
		);
	});
	...
	it("reconciles a symlinked project directory empty without reading its target", async (t) => {
		...
		assert.strictEqual(counters.removed, 1);
		assert.deepStrictEqual(
			getMemories(dbManager, { project: "linked-project", target: "memory" }),
			[],
		);
	});
	...
	it("reconciles a symlinked project memory file empty without reading its target", async (t) => {
		...
		try {
			...
			if ((error as NodeJS.ErrnoException).code === "EPERM")
				return t.skip("file symlinks unavailable");
			throw error;
		}
		addMemory(
			dbManager,
	...
	it("migrates a populated legacy database before startup reconciliation", async () => {
		...
		const targetDir = path.join(agentRoot, "gsd-pi-hermes-memory");
	...
	it("does not create a destination database after critical migration failure", async () => {
		...
		fs.mkdirSync(legacyDir, { recursive: true });
		fs.writeFileSync(
			path.join(legacyDir, "sessions.db"),
	...
	it("hands a corrupt legacy database to bounded destination recovery", async () => {
		...
		try {
			...
			assert.deepStrictEqual(targetManager.getDb().pragma?.("quick_check"), [
				{ quick_check: "ok" },
			]);
			assert.equal(
				targetManager.getLastRecovery()?.strategy,
				"recreated-empty",
			);
			assert.ok(
				fs
					.readdirSync(targetDir)
					.some((name) => name.startsWith("sessions.db.corrupt-")),
	...
	it("resolves a migrated file-symlink database at first I/O", {
		...
		fs.symlinkSync(
			path.relative(legacyDir, realDbPath),
			path.join(legacyDir, "sessions.db"),
			"file",
		);
		const targetManager = new DatabaseManager(targetDir);
		...
		try {
			...
			assert.equal(
				fs.lstatSync(path.join(targetDir, "sessions.db")).isSymbolicLink(),
				true,
			);
