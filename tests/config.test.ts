...
describe("loadConfig", () => {
	...
	it("honors a configured consolidationTimeoutMs, warning only when it is below the default", () => {
		...
		try {
			fs.writeFileSync(
				TEST_CONFIG_PATH,
				JSON.stringify({ consolidationTimeoutMs: 300000 }),
			);
			assert.strictEqual(
			...
			assert.deepStrictEqual(
				warnings,
				[],
				"a value above the default should not warn",
			);

			fs.writeFileSync(
				TEST_CONFIG_PATH,
				JSON.stringify({ consolidationTimeoutMs: 60000 }),
			);
			assert.strictEqual(
				loadConfig(TEST_CONFIG_PATH).consolidationTimeoutMs,
				60000,
			...
			assert.strictEqual(
				warnings.length,
				1,
				"a sub-default value should warn once",
			);
	...
	it("ignores invalid child extension path configuration", () => {
		fs.writeFileSync(
			TEST_CONFIG_PATH,
			JSON.stringify({ childExtensionPaths: ["/tmp/auth.ts", 42] }),
		);
		assert.strictEqual(
			loadConfig(TEST_CONFIG_PATH).childExtensionPaths,
			undefined,
		);
	});
	...
	it("expands ~/ memoryDir into an absolute home path", () => {
		...
		assert.strictEqual(
			config.memoryDir,
			path.join(os.homedir(), ".gsd", "agent", "gsd-pi-hermes-memory"),
		);
	});
	...
	it("resolves relative memoryDir values against the agent root instead of cwd", () => {
		...
		assert.strictEqual(
			config.memoryDir,
			path.join(AGENT_ROOT, "custom-memory-root"),
		);
	});
	...
	it("accepts valid reviewTransport values and ignores invalid ones", () => {
		...
		for (const transport of ["direct", "subprocess"] as const) {
			fs.writeFileSync(
				TEST_CONFIG_PATH,
				JSON.stringify({ reviewTransport: transport }),
			);
			const config = loadConfig(TEST_CONFIG_PATH);
		...
		fs.writeFileSync(
			TEST_CONFIG_PATH,
			JSON.stringify({ reviewTransport: "branch" }),
		);
