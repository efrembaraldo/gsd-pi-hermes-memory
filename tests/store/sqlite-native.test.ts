...
describe("sqlite-native loader", () => {
	...
	it("rebuilds once on ABI mismatch then succeeds", () => {
		...
		const req = fakeRequire(
			...
			(id: string) => {
				assert.equal(id, "better-sqlite3");
				return path.join(
					"/tmp/fake-npm/node_modules/better-sqlite3/lib/index.js",
				);
			},
		);
	...
	it("throws BetterSqlite3LoadError with recovery guidance when rebuild fails", () => {
		...
		assert.throws(
			...
			(error: unknown) => {
				...
				assert.match(
					message,
					/gsd-pi-hermes-memory could not load the native better-sqlite3 module/,
				);
				...
				assert.equal(
					error.packageRoot,
					"/tmp/fake-npm/node_modules/better-sqlite3",
				);
				return true;
			},
		);
	...
	it("formats actionable ABI recovery text", () => {
		...
		assert.match(
			message,
			/\/Users\/me\/\.pi\/agent\/npm\/node_modules\/better-sqlite3/,
		);
		assert.match(message, /npm rebuild better-sqlite3/);
		assert.match(message, /Homebrew/);
	});
